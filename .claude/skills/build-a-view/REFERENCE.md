...
Live example: `config/sync/block.block.guardrails_designer_games.yml`
(and `…_publisher_games.yml`).
...
Live implementation: `web/modules/custom/board_games/src/Plugin/views/filter/PlayerCount.php`
and the `hook_views_data()` in `board_games.module`.
...
Live examples: `tests/src/Kernel/RelatedGamesViewTest.php` (argument),
`GameFinderViewTest.php` and `GameFinderFilterTest.php` (exposed + custom filter)
in `web/modules/custom/board_games/`.
