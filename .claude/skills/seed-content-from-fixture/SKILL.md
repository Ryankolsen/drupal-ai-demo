...
[ dev-time, once ]          [ committed ]        [ runtime, repeatable ]
  bg:fetch  ──writes──▶   fixtures/games.json   ──read by──▶  bg:seed
...
from seeding: a second Drush command (`bg:fetch`) calls the API and **writes the
...
services:
  board_games.fetcher:
    class: Drupal\board_games\BggFetcher
    arguments: ['@http_client', '@logger.factory']
  Drupal\board_games\BggFetcherInterface: '@board_games.fetcher'
```
